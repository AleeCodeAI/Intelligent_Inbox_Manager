...
def evaluate_retrieval(test: TestQuestion, k: int = 10) -> RetrievalEval:
    ...
    print(len(retrieved_docs), "documents retrieved for evaluation.")
...
def evaluate_answer(test: TestQuestion) -> Tuple[AnswerEval, str, List]:
    ...
    # Step 2: LLM judge prompt (force strict JSON output)
    judge_messages = [
        {
            "role": "system",
            "content": (
...
Provide detailed feedback and return your response in **strict JSON** like this:
...
Do not include any extra text, markdown, or formatting."""
    ...
    judge_response = openrouter.chat.completions.create(
        model=MODEL,
    ...
    # Step 4: Parse JSON into AnswerEval
    content = judge_response.choices[0].message.content
    # In case the LLM returns extra whitespace/newlines, strip it
