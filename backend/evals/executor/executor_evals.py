...
from core.executor_observability import ExecutorObservability
...
class ExecutorEvaluator(Logger):
    name: str = "ExecutorEvaluator"
    ...
    def __init__(self):
        ...
        self.log("ExecutorEvaluator Initialized!")
    ...
    def evaluate_sample(self, data: dict) -> dict:
        ...
        if result.classification not in ["BASIC", "PRIORITY", "NON_BUSINESS"]:
            failures.append(f"Invalid classification: {result.classification}")
        if not (0.0 <= result.confidence <= 1.0):
            failures.append(f"Confidence out of range: {result.confidence}")
        if result.classification != data["expected_classification"]:
            failures.append(
                f"Classification mismatch: expected={data['expected_classification']}, got={result.classification}"
            )
