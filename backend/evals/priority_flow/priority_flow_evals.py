...
from schemas import InboundEmail, PriorityResult, SampleResult, ClassStats
import logging
...
class PriorityFlowEvaluator(Logger):
    ...
    def __init__(self, data_path: Path, output_dir: Path):
        ...
        self.results_file = output_dir / f"priority_flow_results_{self.run_id}.jsonl"
        self.summary_file = output_dir / f"priority_flow_summary_{self.run_id}.md"
    ...
    def evaluate_sample(self, data: dict) -> SampleResult:
        ...
        result: PriorityResult = self.flow._call_llm(email=email, obs=self.obs)

        sample_result = SampleResult(
            gmail_id=data["gmail_id"],
            subject=data["subject"],
            expected=data["expected_classification"],
            predicted=result.priority_type,
    ...
    def write_summary(self, results: list[SampleResult], correct: int, class_stats: dict[str, ClassStats], confusion: dict[str, dict[str, int]]):
        ...
        md = []
        md.append("# Priority Flow Evaluation\n")
        ...
        md.append("\n---")
        md.append(f"_Generated by PriorityFlowEvaluator — Run `{self.run_id}`_")
...
if __name__ == "__main__":
    evaluator = PriorityFlowEvaluator(
        data_path=Path(__file__).parent.parent / "data" / "priority_emails_test_data.jsonl",
