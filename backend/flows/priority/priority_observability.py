...
class PriorityFlowObservability:
    """
    Langfuse observability for PriorityFlow.
    ...
    # ------------------------------------------------------------------
    # 1. Trace — one per email, opened at the start of PriorityFlow.run()
    # ------------------------------------------------------------------
        ...
        self._ingest([
            {
                ...
                "body": {
                    "id": self._trace_id,
                    "name": "priority-flow-email",
                    "sessionId": thread_id,
                    "input": body,
                    "tags": ["priority-flow", "email"],
                    "metadata": {
                        "gmail_id": gmail_id,
    ...
    def finish_trace(self, result: PriorityResult) -> None:
    ...
    def score_success(self, comment: str = "Priority classified successfully.") -> None:
        self._score("priority-flow-success", 1, comment)

    def score_failure(self, reason: str) -> None:
        self._score("priority-flow-failure", 0, reason)
