...
# Page Configuration
st.set_page_config(
    page_title="Basic Emails Manager",
    page_icon="📧",
    layout="wide"
)

# 🔮 Custom CSS with Animations (UI ONLY)
st.markdown("""
<style>
...
.stTextInput input:focus {
    ...
    box-shadow: 0 0 12px rgba(155, 92, 255, 0.45);
}
...
# Header Section
st.title("Basic Emails – Manual Response Queue")
st.markdown("### Emails Requiring Personal Attention")
st.markdown("""
This queue contains basic emails that could not be automated due to insufficient information.
These emails require manual review and personalized responses.
""")
st.markdown("---")
...
def fetch_emails():
    try:
        response = requests.get(f"{API_BASE_URL}/basic/emails")
            ...
            if send_clicked:
                if not crafted_message.strip():
                    ...
                    with st.spinner("Sending..."):
                        ...
                        if success and str(result.get("status","")).lower() == "success":
